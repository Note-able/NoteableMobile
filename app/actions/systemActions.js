...
export const runBackgroundRequests = () => (dispatch, getState) => {
  BackgroundFetch.configure(
    ...
    () => {
      const { SystemReducer } = getState();
      if (
        SystemReducer.network.connected !== 'none' &&
        SystemReducer.network.queue != null &&
        SystemReducer.network.queue.length !== 0
      ) {
