...
import { Animated, Easing, FlatList, TouchableHighlight, View } from 'react-native';
...
export default class Recordings extends Component {
  ...
  componentDidMount() {
    if (this.props.recordings.length !== 0) {
      Animated.timing(this.state.recordingsOpacity, {
        easing: Easing.cubic,
        toValue: 1,
        duration: 200,
      }).start();
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (
      this.props.recordings != null &&
      this.props.recordings.order.length === 0 &&
      nextProps.recordings.order.length !== 0
    ) {
      Animated.timing(this.state.recordingsOpacity, {
        easing: Easing.cubic,
        toValue: 1,
  ...
  render() {
    return (
      <FlatList
        ...
        renderItem={({ item }) => {
          const recording =
            this.state.recordings.local[item] || this.state.recordings.networked[item];
          return (
            <View key={recording.id}>
              ...
              >
                <TouchableHighlight
                  ...
                  onPress={() => this.props.deleteRecording(recording)}
                >
                  <Icon name="delete" size={25} color={colors.shade90} />
                </TouchableHighlight>
                <TouchableHighlight
                  style={{ width: 25, height: 25, margin: 10 }}
                  onPress={() => this.props.editRecording(recording)}
                >
                ...
                {recording.path !== '' && recording.isSynced ? (
                  <TouchableHighlight
                    style={{ width: 25, height: 25, margin: 10 }}
                    onPress={() => this.props.removeRecording(recording)}
                  >
                  ...
                  <TouchableHighlight
                    ...
                    onPress={() =>
                      recording.path === ''
                        ? this.props.downloadRecording(recording)
                        : this.props.uploadRecording(recording, this.props.currentUser)
                    }
                  >
                    <Icon
                      name={recording.path === '' ? 'file-download' : 'cloud-upload'}
                      size={25}
                      color={this.props.currentUser == null ? colors.shade40 : colors.shade90}
                    />
                  </TouchableHighlight>
                )}
              </View>
              ...
              >
                <Recording
                  ...
                  name={recording.name}
                  isOpen={
                    this.state.showOptions != null && this.state.showOptions.id === recording.id
                  }
